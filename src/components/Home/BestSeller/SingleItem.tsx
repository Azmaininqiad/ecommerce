...
const SingleItem = ({ item }: { item: Product }) => {
  ...
  const handleAddToCart = () => {
    const { discounted_price, ...restOfItem } = item;
    dispatch(
      addItemToCart({
        ...restOfItem,
            // 3. Add the renamed property, checking if it exists
  ...
  const handleItemToWishList = () => {
    // 1. Destructure the item, renaming discounted_price to discountedPrice
    //    and capture the rest of the properties in 'restOfItem'.
    const { discounted_price, ...restOfItem } = item;

    dispatch(
      addItemToWishlist({
        // 2. Spread the rest of the item's properties
        ...restOfItem,
        // 3. Add the correctly named 'discountedPrice' property
        discountedPrice: discounted_price, 
        // 4. Add your other new properties
        status: "available",
        quantity: 1,
      })
    );
      ...
      <div className="relative overflow-hidden rounded-lg bg-[#F6F7FB] min-h-[403px]">
        <div className="text-center px-4 py-7.5">
          ...
          <span className="flex items-center justify-center gap-2 font-medium text-lg">
            <span className="text-dark">${item.discounted_price}</span>
            <span className="text-dark-4 line-through">${item.price}</span>
          </span>
        </div>

        <div className="flex justify-center items-center">
          <Image src={item.imgs.previews[0] || null} alt="" width={280} height={280} />
        </div>
